...
namespace BmpHelper
{
     ...
     /*  public static void AplicarSombra(string bmpFilePath, string layerFilePath, string outputFilePath)
        {
            ...
            using (Bitmap layerImage = new Bitmap(layerFilePath))
            {
                ...
                System.Drawing.Color transparentColor = Color.Black;
                ...
                {
                    g.DrawImage(layerImage, new System.Drawing.Rectangle(0, 0, baseImageDirect.Width, baseImageDirect.Height), 0, 0, baseImageDirect.Width, baseImageDirect.Height, GraphicsUnit.Pixel, imageAttributes);
                }
     ...
     */
        //public static void CrearSombras8BPP(string camisetaPath, string sombreaPath, string camisetaSomreadaPath)System.Drawing.
        //{
        ...
        private static System.Drawing.Color BytesToColor(byte[] data)
        {
            System.Drawing.Color color = new System.Drawing.Color();

            color = System.Drawing.Color.FromArgb(data[3], data[2], data[1], data[0]);
        ...
        public static System.Drawing.Color[] LoadColors(string bmpPath)
        {
            ...
            System.Drawing.Color[] color = new System.Drawing.Color[data.Length / 4];
            int colorIndex = 0;
            for (int i = 0; i < data.Length; i += 4)
            {
                ...
                dataTemp[2] = data[i + 2];
                dataTemp[3] = 0xFF;
                color[colorIndex] = System.Drawing.Color.FromArgb(dataTemp[3], dataTemp[2], dataTemp[1], dataTemp[0]);
                colorIndex++;
            }
            return color;
        }
        ...
        public static void SetColors(string bmpPath, System.Drawing.Color[] colors)
        {
        ...
        private static byte[] SetColorToBytes(System.Drawing.Color[] colors)
        {
        ...
        {
            // Convertir a 4bpp Indexed con ImageSharp
            using (Image<Rgba32> img = SixLabors.ImageSharp.Image.Load<Rgba32>(ToByteArray(image)))
            {
                ...
                // Guardar como BMP de 4 bits
                img.SaveAsBmp(stream, new BmpEncoder { BitsPerPixel = BmpBitsPerPixel.Pixel4 });
            }
        }
        ...
        // Método para guardar el contenido de un PictureBox en un MemoryStream
        public MemoryStream PictureBoxToStream(PictureBox pictureBox)
        {
            if (pictureBox.Image == null)
                throw new ArgumentException("El PictureBox no tiene una imagen.");

            // Crear un MemoryStream para almacenar la imagen
            MemoryStream stream = new MemoryStream();

            // Guardar la imagen del PictureBox en el MemoryStream en formato PNG
            pictureBox.Image.Save(stream, ImageFormat.Png);

            // Colocar el cursor al inicio del MemoryStream para lectura posterior
